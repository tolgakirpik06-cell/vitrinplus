...
import { SellerDocumentsPanel } from "@/components/seller-application/SellerDocumentsPanel";
...
const STATUS_COPY = {
  ...
  rejected: { icon: ShieldX, title: "Başvurun reddedildi", text: "Aşağıdaki nedeni inceleyip eksikleri giderdikten sonra bilgilerini ve belgelerini güncelleyerek yeniden başvurabilirsin.", tone: "bg-rose-50 text-rose-800" },
...
function LiveStatus() {
  ...
  return (
    <section className="mx-auto max-w-2xl space-y-6 rounded-2xl border border-navy-100 bg-white p-6 sm:p-8">
      <h1 className="text-2xl font-bold">Başvuru durumu</h1>
      <div className={`flex items-start gap-3 rounded-xl p-4 text-sm ${copy.tone}`}>
        ...
        <div>
          ...
          {sellerAccount.rejectionReason && (
            <p className="mt-3 rounded-lg bg-white/70 p-3 text-sm"><span className="block text-xs font-semibold uppercase tracking-wide opacity-70">{sellerAccount.status === "suspended" ? "Gerekçe" : "Red nedeni"}</span>{sellerAccount.rejectionReason}</p>
          )}
        </div>
      </div>
      <dl className="grid grid-cols-[auto_1fr] gap-x-6 gap-y-2 text-sm">
        ...
        {sellerAccount.reviewedAt && (<><dt className="text-navy-400">Karar tarihi</dt><dd className="font-semibold">{new Date(sellerAccount.reviewedAt).toLocaleDateString("tr-TR")}</dd></>)}
      </dl>
      <SellerDocumentsPanel sellerType={sellerAccount.sellerType} status={sellerAccount.status} />
