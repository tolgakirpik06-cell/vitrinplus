...
export function EarningsCard() {
  ...
  return mode === "supabase" ? <LiveEarningsCard /> : <DemoEarningsCard />;
}

/** Gerçek mod: tüm tutarlar kazanç defterinden gelir (komisyon %0, ödeme sağlayıcısı kesintisi henüz yok). */
function LiveEarningsCard() {
  const { finance, financeError, payouts } = useSellerWorkspace();
  ...
  return (
    <Panel aria-label="Kazançlarım">
      ...
      />
      {financeError ? (
        <p role="alert" className="text-sm text-rose-600">Kazanç özeti yüklenemedi. {financeError}</p>
      ) : !summary ? (
        <p role="status" className="text-sm text-muted">Kazanç özeti yükleniyor…</p>
      ) : (
        <div className="grid gap-4 md:grid-cols-[1.15fr_1fr]">
          <div>
            <Line label="Brüt satış" value={summary.grossSales} />
            <Line label="Satıcı indirimi" value={summary.sellerDiscounts} negative />
            <Line label="İadeler" value={summary.returns} negative />
            <Line label="Ödeme hizmeti kesintisi" value={summary.paymentProviderDeduction} negative />
            <Line label="VitrinPlus satış komisyonu" value={summary.commission} extra={<StatusBadge tone="success" dot={false}>{COMMISSION_LABEL.replace("satış ", "")}</StatusBadge>} />
          </div>
          <div className="flex flex-col gap-3">
            <div className="rounded-xl border border-royal-100 bg-royal-50/60 p-4">
              <p className="text-xs font-semibold text-navy-600">Net Kazanç</p>
              <p className="mt-1 text-[26px] font-extrabold leading-none tracking-tight text-royal-700 tabular-nums">{formatTL(summary.net)}</p>
              <p className="mt-2 text-xs text-muted">Ödemeye hazır: {formatTL(summary.availableBalance)}</p>
            </div>
